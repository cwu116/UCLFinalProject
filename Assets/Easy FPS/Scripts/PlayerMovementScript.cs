...
public class PlayerMovementScript : MonoBehaviour {
    ...
    void Awake(){
		...
		control.Move.Movement.performed += ctx => moveInputVal = ctx.ReadValue<Vector2>();
		control.Move.Movement.canceled += ctx => moveInputVal = Vector2.zero;
	}
	...
	*/
	...
	void PlayerMovementLogic(){
		...
		if (grounded) {
            ...
            rb.AddRelativeForce (moveInputVal.x * accelerationSpeed * Time.deltaTime, 0, moveInputVal.y * accelerationSpeed * Time.deltaTime);
		} else {
            float tempHorizontal = Input.GetAxis("Horizontal");
            float tempVertical = Input.GetAxis("Vertical");
            rb.AddRelativeForce (tempHorizontal * accelerationSpeed / 2 * Time.deltaTime, 0, tempVertical * accelerationSpeed / 2 * Time.deltaTime);
