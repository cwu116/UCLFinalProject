...
public class GunScript : MonoBehaviour {
	...
	private XboxControl control;
	private bool gunPressed;
	...
	void Awake(){
		...
		control.Move.Shoot.performed += ctx => gunPressed = true;
		control.Move.Shoot.canceled += ctx => gunPressed = false;
        control.Move.Zoom.performed += ctx => zoomPressed = true;
        control.Move.Zoom.canceled += ctx => zoomPressed = false;
	...
	*/
	void FixedUpdate(){
        ...
        if (zoomPressed && !reloading && !meeleAttack)
        {
